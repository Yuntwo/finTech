...
// CacheHasCoupon 缓存用户拥有优惠券/商家创建优惠券的信息
func CacheHasCoupon(coupon model.Coupon) (int64, error) {
	...
	val, err := SetAdd(key, coupon.CouponName)
	return val, err
}
...
func CacheCouponAndHasCoupon(coupon model.Coupon) error {
	...
	if user, err := dao.GetUser(coupon.Username); err != nil {
...
func GetCoupon(couponName string) model.Coupon {
	...
	values, err := GetMap(key, "id", "username", "couponName", "amount", "left", "stock", "description")
	if err != nil {
...
func GetCoupons(userName string) ([]model.Coupon, error) {
	...
	couponNames, err := GetSetMembers(hasCouponsKey)
	if err != nil {
